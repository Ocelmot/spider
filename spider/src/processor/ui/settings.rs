...
impl UiProcessorState {
    ...
    pub(crate) async fn add_setting(
        ...
        for (index, (input_type, input_label)) in inputs
            .into_iter()
            .chain(null_inputs.into_iter())
            .take(3)
            .enumerate()
        {
            map.insert(
                format!("input_{}_type", index),
                DatasetData::String(input_type),
            );
            map.insert(
                format!("input_{}_label", index),
                DatasetData::String(input_label),
            );
        }
            ...
            Some((title_map, list)) => {
                ...
                match title_map.get(&title) {
                    Some(index) => {
                        ...
                        // update dataset item
                        let dataset_message = DatasetMessage::SetElement {
                            path: dataset_path,
                            data: data_item,
                            id: *index,
                        };
            ...
            None => {
                ...
                elem.append_child({
                    ...
                    settings_list_element.append_child({
                        ...
                        row.append_child({
                            let input_type =
                                UiElementKind::Variable(UiElementContentPart::Data(vec![
                                    "input_0_type".to_string(),
                                ]));
                            let mut input = UiElement::new(input_type);
                            input.set_id(format!("{}0", header.clone()));
                            input.set_selectable(true);
                            input.set_content(UiElementContent::new_data(
                ...
                self.update_dataset_subscriptions(&abs_dataset_path, 1)
                    .await;
            }
        };
    }
            ...
            Some((title_map, list)) => {
                match title_map.remove(&title) {
                    Some(index) => {
                        ...
                        let dataset_message = DatasetMessage::DeleteElement {
                            path: dataset_path,
                            id: index,
                        };
                        let dataset_processor_message =
