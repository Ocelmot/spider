    ...
    pub fn listen<A: ToSocketAddrs + Send + 'static>(
        ...
        tokio::spawn(async move {
            let listener = TcpListener::bind(listen_addr)
                .await
                .expect("failed to start Link listener");
            loop {
...
impl LinkBuilder {
    ...
    async fn send_stream_config(&mut self) {
        ...
        let other_relation = self
            .other_relation
            .as_ref()
            .expect("stream config can only be sent after setting other_relation");
        let data = {
            // encrypt with other party's key
            let key = other_relation
                .id
                .as_pub_key()
                .expect("Failed to convert to key");
            let mut rng = rand::thread_rng();
            let enc_data = key
                .encrypt(&mut rng, Pkcs1v15Encrypt, &raw_data)
    ...
    async fn read_stream_config(&mut self, enable_key_request: &Option<String>) -> bool {
        ...
        let dec_data = {
            // ensure that encryption items are not held across await
    ...
    async fn read_introduction(&mut self) {
        ...
        let prot: Protocol =
            serde_json::de::from_slice(&dec_data).expect("Failed to deserialize introduction");
        // create Relation
