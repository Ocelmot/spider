...
mod content;
pub use content::{UiElementContent, UiElementContentPart};

mod change;
pub use change::{UiChildOperations, UiElementChange, UiElementChangeSet};
...
/// of element that can be laid out on the page.
...
pub enum UiElementKind {
    ...
    // Input
    /// A text box used for receiving a text input from the user
    TextEntry,
    /// A button used for receiving a click input from the user
    Button,
...
impl UiElement {
    ...
    pub fn render_content_opt(&self, data: &Option<&DatasetData>) -> String {
        match data {
            Some(data) => self.content.resolve(data),
            None => self.text(),
        }
    }
    ...
    pub fn get_child_mut<'a>(&'a mut self, index: usize) -> Option<&'a mut UiElement> {
        match &mut self.children {
            Some(children) => children.get_mut(index),
            None => None,
        }
    }

    /// Returns an iterator of this UiElement's children.
    pub fn children(&self) -> std::slice::Iter<'_, UiElement> {
        match &self.children {
    ...
    pub fn children_dataset<'a>(
        &'a self,
        data: &'a Option<&DatasetData>,
        data_map: &'a HashMap<AbsoluteDatasetPath, Vec<DatasetData>>,
    ) -> UiElementDatasetIterator<'a> {
...
impl<'a> UiElementDatasetIterator<'a> {
    fn new(
        ...
        let back_dataset = match &elem.dataset {
            Some(dataset) => match dataset_map.get(&dataset) {
